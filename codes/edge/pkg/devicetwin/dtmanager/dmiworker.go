...
import (
	...
	"github.com/kubeedge/kubeedge/edge/pkg/devicetwin/config"
...
func (dw *DMIWorker) initDeviceMapperInfoFromDB() {
	...
	if dw.dmiCache != nil {
		// 处理nil的情况，可能需要返回错误或执行其他逻辑
		klog.Infoln("dmiCache not nil")

	}

}

func (dw *DMIWorker) socket_server() {
	if len(os.Args) == 1 {
		fmt.Println("Need socket path")
		return
	}
	socketPath := config.LOCAL_SOCKET_PATH
...
func (dw *DMIWorker) callForDeviceLocal(data []byte) ([]byte, bool) {
	...
	json.Unmarshal(data, &dq)

	id := dq.DeviceID
	klog.Infoln("Received deviceId is (", id, ")")

	dw.dmiCache.DeviceModelMu.Lock()
	device := dw.dmiCache.DeviceList[id]
	dw.dmiCache.DeviceModelMu.Unlock()

	klog.Infoln("DeviceList is: ", dw.dmiCache.DeviceList)
	for key, value := range dw.dmiCache.DeviceList {
		fmt.Printf("  %s: %+v\n", key, value)
	}
	// var deviceA v1beta1.Device

	// jsonV, _ := json.Marshal(deviceV)
	// json.Unmarshal(jsonV, &deviceA)

	if dq.PropertyName != "" {
		klog.Infof("Receive update request for %v", device.Name)
		// var msg model.Message = *model.NewMessage("").BuildRouter("meta", "serviceStub", "twin/device/serviceStub", model.UpdateOperation)
		for i := range device.Status.Twins {
			if device.Status.Twins[i].PropertyName == dq.PropertyName {
				klog.Infof("Update device %s with content: %s\n", device.Name, dq.Value)
				device.Status.Twins[i].ObservedDesired.Value = dq.Value
				var updateMsg dmiserver.DeviceTwinUpdate
				updateMsg.BaseMessage.Timestamp = time.Now().UnixNano() / 1e6
				updateMsg.Twin = map[string]*types.MsgTwin{}
				updateMsg.Twin[device.Status.Twins[i].PropertyName] = &types.MsgTwin{}
				updateMsg.Twin[device.Status.Twins[i].PropertyName].Expected = &types.TwinValue{Value: &device.Status.Twins[i].ObservedDesired.Value}

				msg, _ := json.Marshal(updateMsg)
				klog.Infof("Update device %s with content: %s\n", device.Name, string(msg))
				Updated(dw.DTContexts, id, msg)

			}
		}

		// var msg model.Message = *model.NewMessage("").BuildRouter("meta", "twin", device.Name, model.UpdateOperation)
		// content, _ := json.Marshal(device)
		// klog.Infof("Update device %s with content: %s", device.Name, string(content))
		// msg.FillBody(content)
		// dw.DTContexts.Send(dtcommon.DeviceTwinModule, dtcommon.TwinUpdate, dtcommon.DeviceTwinModule, &msg)
	...
	if device != nil {
		if dq.PropertyName == "" {
			klog.Info("Successfully find devices, send info to appmessager")
		}
		...
		data.Location = device.Spec.NodeName
		if timestamp, ok := device.Spec.Protocol.ConfigData.Data["timestamp"].(int64); ok {
			// timestamp 存在，进行赋值
			data.LastOnline = timestamp
		} else {
			// timestamp 不存在，可以选择采取其他措施
			data.LastOnline = 1702444182
		}

		// deviceR := dmiworker.GetDevice(id)
		for _, v := range device.Status.Twins {
			data.Values[v.PropertyName] = v.Reported.Value
		}
		res, _ := json.Marshal(data)
		return res, true
	}

	klog.Info("Failed fild device (%v) local", id)
	return nil, false
}

func (dw *DMIWorker) echo(c net.Conn) {
	for {
		buf := make([]byte, 128)
		n, err := c.Read(buf)
		if err != nil {
		...
		resData, _ := dw.callForDeviceLocal(data)
		if resData != nil {
			_, err = c.Write(resData)
			if err != nil {
