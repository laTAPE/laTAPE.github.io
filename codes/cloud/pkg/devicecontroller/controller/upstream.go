	...
	for {
		...
		case msg := <-uc.deviceStatusChan:
			...
			for twinName, twin := range msgTwin.Twin {
				// klog.Infof("Twin name: %s, twin expect: %v, twin actual: %v", twinName, *twin.Actual.Value, *twin.Expected.Value)
				for i, cacheTwin := range deviceStatus.Status.Twins {
					// new: add expected value to device status
					if twinName == cacheTwin.PropertyName && twin.Expected != nil && twin.Expected.Value != nil {
						expected := v1beta1.TwinProperty{}
						expected.Value = *twin.Expected.Value
						expected.Metadata = make(map[string]string)
						if twin.Expected.Metadata != nil {
							expected.Metadata["timestamp"] = strconv.FormatInt(twin.Expected.Metadata.Timestamp, 10)
						}
						if twin.Metadata != nil {
							expected.Metadata["type"] = twin.Metadata.Type
						}
						deviceStatus.Status.Twins[i].ObservedDesired = expected
					}
